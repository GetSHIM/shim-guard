"""IBAN country format patterns.

Copied verbatim from presidio-analyzer 2.2.364
(``presidio_analyzer/predefined_recognizers/generic/iban_patterns.py``),
which is distributed under the MIT licence by Microsoft Corporation. Kept
byte-identical so IBAN format validation does not drift from the behaviour it
replaces; regenerate rather than edit.
